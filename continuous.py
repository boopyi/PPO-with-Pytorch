...
class Actor(nn.Module):
    def __init__(self):
        ...
        obs_dim = env.single_observation_space["obs"].shape[0]
        total_dim = obs_dim + self.img_enc.out_dim
        ...
        self.log_std_head = nn.Linear(512, env.single_action_space.shape[0])
    ...
    def forward(self, obs, image):
        ...
        x = torch.cat([obs, img_feat], dim=1)

        x = self.net(x)
        mu = torch.tanh(self.mu_head(x)) * self.high
        log_std = torch.clamp(self.log_std_head(x), -4, -1)
...
if __name__ == '__main__':
    ...
    def update_policy(s_obs,s_image, actions, log_probs_old, returns, advantages, batch_size=512, epochs=10):
        ...
        for epoch in range(epochs):
            ...
            for start in range(0, dataset_size, batch_size):
                ...
                actor_total_loss = policy_loss - (0.001 * entropy) 
                ...
                value_pred = value_func(obs_b,image_b).squeeze()
    ...
    if  task=="train":
        try:
            episode_count = 0
            total_steps = 0
            storage_obs = torch.zeros((1024, num_envs, env.single_observation_space["obs"].shape[0]), device=device)
            storage_image=torch.zeros((1024, num_envs, env.single_observation_space["image"].shape[0], env.single_observation_space["image"].shape[1], env.single_observation_space["image"].shape[2]), device=device)
            for _ in range(num_episodes):
                ...
                for i in range(1024):
                    ...
                    storage_image[i]=torch.from_numpy(obs["image"]).to(device, dtype=torch.float32)
                    if i % 100 == 0:
                        print(f"Step {i}/1024...", end="\r")
                    with torch.no_grad():
                        curr_image=storage_image[i]

                        storage_obs[i]=normilizer.update(storage_obs[i])
                        mu, std = policy.forward(storage_obs[i],curr_image)
                        dist = torch.distributions.Normal(mu, std)
                        action = dist.sample()
                        
                        
                        log_prob = dist.log_prob(action).sum(dim=-1)
                        value = value_func.forward(storage_obs[i],storage_image[i])
                        obses.append(storage_obs[i].cpu())
                        images.append(storage_image[i].cpu())
                ...
                with torch.no_grad():
                    ...
                    final_image_tensor = torch.from_numpy(obs["image"]).float().to(device)
                    variance = normilizer.m2 / normilizer.k
                    std = torch.sqrt(variance + 1e-8)
                    normed_last_obs = torch.clamp((final_obs_tensor - normilizer.mean) / std, -5, 5)
                    
                   
                    last_value = value_func.forward(normed_last_obs,final_image_tensor).squeeze()
                ...
                images_tensor= torch.stack(images).to(device).view(-1, env.single_observation_space["image"].shape[0],env.single_observation_space["image"].shape[1],env.single_observation_space["image"].shape[2])
                ...
                update_policy(obses_tensor,images_tensor, actions_tensor, log_probs_tensor, returns_flat, advantages_flat)
        ...
        policy.eval()
        mean = checkpoint["obs_mean"].flatten()
        m2 = checkpoint["obs_m2"].flatten()
        k = checkpoint["obs_k"]
        ...
        for i in range(1000):
            ...
            obs_tensor= torch.from_numpy(obs["obs"]).float().to(device)
            obs_tensor=(obs_tensor-mean)/std
            obs_tensor=torch.clamp(obs_tensor, -5, 5).unsqueeze(dim=0)
            image_tensor=torch.from_numpy(obs["image"]).to(device, dtype=torch.float32).unsqueeze(0)
            with torch.no_grad():
                
                mu,stdd = policy(obs_tensor,image_tensor)
                action=mu
